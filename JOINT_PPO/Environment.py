...
class GridWorldEnvironment(ParallelEnv):
    ...
    # Reward Weights: align to MAPPO style for coordinated exploration and team incentives
    # Note: rewardProximity is dynamically scaled in __init__ based on map size
    rewardWeight = {
        "tileDiscovered": 0.5,  # Reduced from 2.0 - exploration should be secondary
        "individualDiscovery": 1.5,  # Reduced from 6.0 - still incentivize new discoveries
        "rewardFound": 500.0,  # Increased from 100.0 - make finding reward highly rewarding
        "rewardProximity": 10.0,  # Base value (15x15): scaled by map_size/15 in __init__
        "HazardHit": -100.0,
        "Steps": -0.005,  # Reduced step penalty to encourage longer exploration
    ...
    def __init__(
        ...
        # Create instance copy of rewardWeight to avoid modifying class variable
        self.rewardWeight = self.rewardWeight.copy()
        
        # Scale rewardProximity by map size for fair navigation guidance across different map sizes
        # Larger maps have greater distances, so proximity rewards need to scale accordingly
        map_size_scale = self.grid_size / 15.0  # 15x15 is baseline
        self.rewardWeight["rewardProximity"] = 10.0 * map_size_scale  # Use base value, not accumulated
        log.d(f"Scaled rewardProximity to {self.rewardWeight['rewardProximity']:.2f} for map size {self.grid_size}x{self.grid_size}")
        ...
        self.reward_position = None  # Track reward position for distance calculations
    ...
    def reset(self, seed=None, options=None):
        ...
        # Initialize previous distances for distance-based rewards
        self.prev_distances = {}
        for agent in self.agents:
            if self.reward_position is not None:
                x, y = self.agent_positions[agent]
                rx, ry = self.reward_position
                self.prev_distances[agent] = abs(x - rx) + abs(y - ry)
    ...
    def step(self, actions):
        ...
        rewards = {agent: self.rewardWeight["Steps"] for agent in self.agents}  # Use dict-based step penalty
        ...
        # Distance-based rewards (applied before other rewards)
        for agent in self.agents:
            if agent in self.prev_distances:
                prev_dist = self.prev_distances[agent]
                distance_reward = self._calculate_distance_reward(agent, prev_dist)
        ...
        for agent in self.agents:
            ...
            for x, y in self._visible_tiles(agent):
                ...
                if not self.discovered[x, y]:
                    self.discovered[x, y] = True
                # Reward agent for tiles new to them (apply both base and individual bonus)
                if not self.agent_discovered[agent][x, y]:
                    self.agent_discovered[agent][x, y] = True
                    newly_discovered += 1
                    rewards[agent] += self.rewardWeight["individualDiscovery"]  # Individual bonus per new tile
        ...
        # Reward tiles - agents get reward but episode ends when found (to match IPPO/MAPPO step reduction behavior)
        reward_collected = False
        for agent in self.agents:
            x, y = self.agent_positions[agent]
            if self.grid[x, y] == 3:
                rewards[agent] += self.rewardWeight["rewardFound"]
                self.grid[x, y] = 0  # Remove reward from map
                self.reward_found = True
                reward_collected = True
                log.i(f"{agent} collected reward at ({x}, {y})")
        
        # If reward was collected, end episode for all agents
        if reward_collected:
            for agent in self.agents:
                truncations[agent] = True

        # Hazard penalty - agents get penalty but episode continues (for joint training)
        for agent in self.agents:
            x, y = self.agent_positions[agent]
            if self.grid[x, y] == 2:
                rewards[agent] += self.rewardWeight["HazardHit"]
                # Note: Agent is NOT terminated - they can continue exploring

        # Check if no rewards remain - truncate all agents (like IPPO/MAPPO)
    ...
    def _calculate_distance_reward(self, agent, prev_distance):
        ...
        return distance_change * self.rewardWeight["rewardProximity"]
    ...
    def _place_random_reward(self):
        ...
        if valid_positions:
            ...
            self.grid[rx, ry] = 3
            self.reward_position = (rx, ry)  # Track reward position
