...
from Archives.JOINT_PPO.train_JOINT_PPO import trainAgents
from Archives.JOINT_PPO.simulate_JOINT_PPO import runSimulations
from PlotAnalysis import plotAnalysisData
import json

timesteps_per_iteration = 50_000
total_iterations = 200
num_drones = 2  # Must match the trained model (existing model is 8 drones)

# Get JOINT_PPO directory path
joint_ppo_dir = os.path.dirname(os.path.abspath(__file__))

# Load or initialize progress tracker
progress_file = os.path.join(joint_ppo_dir, "training_progress.json")
...
for iteration in range(start_iteration, total_iterations):
    ...
    # Train
    trainAgents(timesteps_per_iteration, parallel=False, num_drones=num_drones, cumulativeTimestepsSoFar=total_timesteps_so_far)
    
    # Simulate and analyze
    runSimulations(simulations=100, timeStepsRan=current_timesteps, num_drones=num_drones)
    
    # Plot analysis data for all maps
    plotAnalysisData(os.path.join(joint_ppo_dir, "map_15x15_analysis_Results.csv"))
    plotAnalysisData(os.path.join(joint_ppo_dir, "map_30x30_analysis_Results.csv"))
    plotAnalysisData(os.path.join(joint_ppo_dir, "map_45x45_analysis_Results.csv"))
